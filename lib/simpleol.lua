...
function simpleol.screenPrint(mode, x, y, text, size, col1, col2)
    simpleol.init()
    if transp == nil then -- If not given transperency,
        transp = 255 -- We set it to visible (max transperency)
    end
    ...
    if mode == 0 then -- Regular Mode (__ALEFT)
        screen.print(x, y, text, 0.1 + size, col1, col2, __ALEFT) -- We just draw the text
    elseif mode == 1 then -- Centered Mode (__ACENTER)
...
function simpleol.saveIntoFile(path, value)
...
function simpleol.loadFromFile(path)
